...
export default function Navbar() {
  ...
  // Subtle shadow once the page is scrolled — navbar itself stays pinned (sticky).
  useEffect(() => {
  ...
  // Close the mobile drawer if the viewport is resized past the mobile breakpoint.
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* ── Mobile Search Bar (expands when search icon clicked) ── */}
      {mobileSearchOpen && (
        <div className="border-b border-silver-light bg-white px-3 py-2 md:hidden">
          <form onSubmit={(e) => { handleSearch(e); setMobileSearchOpen(false); }}
            className="flex items-center overflow-hidden rounded-full border border-royal bg-white shadow-sm">
      ...
      <div
        className={`overflow-hidden border-b border-silver-light bg-white transition-all duration-300 md:hidden ${
          mobileCatOpen ? "max-h-64" : "max-h-0 border-b-0"
        }`}
      >
      ...
      <div className="hidden border-b border-silver-light bg-white shadow-sm md:block">
        <div className="container-page flex items-stretch">
          <div className="relative"
            ...
            <button className="flex h-full items-center gap-2 bg-navy px-5 py-3 text-sm font-semibold text-white hover:bg-navy-dark transition-colors duration-150">
              <AlignJustify size={16} /> All Categories
