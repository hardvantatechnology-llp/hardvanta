        ...
        <div className="mt-6">
          {forgotStage !== "done" && (
            <button
              ...
              className="flex items-center gap-1.5 text-sm text-white/50 hover:text-electric-light mb-6 transition-colors"
            >
          ...
          {forgotInfo && (
            <p className="mb-4 rounded-lg bg-electric/10 border border-electric/20 px-3 py-2 text-sm text-electric-light">{forgotInfo}</p>
          )}
          {forgotError && (
            <p className="mb-4 rounded-lg bg-red-500/10 border border-red-500/20 px-3 py-2 text-sm text-red-400">{forgotError}</p>
          )}
            ...
            {forgotStage === "email" && (
              <motion.div {...motionProps("email")}>
                <form onSubmit={handleForgotPassword} className="space-y-4">
                  ...
                  <Button type="submit" variant="gradient" size="lg" className="w-full" disabled={forgotLoading}>
              ...
              <motion.div {...motionProps("reset")}>
                <form onSubmit={handleResetConfirm} className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowNewPassword((v) => !v)}
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-white/30 hover:text-electric-light">
                      {showNewPassword ? <EyeOff size={16} /> : <Eye size={16} />}
                    </button>
                  </div>
                  <Button type="submit" variant="gradient" size="lg" className="w-full" disabled={forgotLoading || forgotCode.length < 6}>
                ...
                <p className="mt-4 text-center text-xs text-white/40">
                  ...
                  <button type="button" onClick={handleResendCode} disabled={forgotLoading}
                    className="font-semibold text-electric-light hover:underline disabled:opacity-50">
                    Resend
                  </button>
                </p>
              </motion.div>
    ...
    >
      <div className="mt-6">
          ...
          {step === "password" ? (
            <motion.div {...motionProps("password")}>
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-3 rounded-xl glass-card py-2.5 text-sm font-semibold text-white hover:shadow-glow-electric transition-all"
              >
              ...
              {info && <p className="mb-4 rounded-xl bg-electric/10 border border-electric/20 px-3 py-2.5 text-sm text-electric-light">{info}</p>}
              {error && <p className="mb-4 rounded-xl bg-red-500/10 border border-red-500/20 px-3 py-2.5 text-sm text-red-400">{error}</p>}
              ...
              <p className="mt-6 text-center text-sm text-white/50">
                ...
                <Link href="/register" className="font-semibold text-electric-light hover:underline">Create one</Link>
              </p>
            </motion.div>
          ) : (
              ...
              <form onSubmit={handleOtpSubmit} className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    placeholder="000000"
                    className="w-full rounded-xl glass-card px-3 py-3 text-center text-2xl font-bold tracking-[0.6em] text-white outline-none focus:shadow-glow-electric transition-all placeholder:text-white/20"
                  />
                </div>

                <Button type="submit" variant="gradient" size="lg" className="w-full" disabled={loading || otp.length < 6}>
                  {loading ? "Verifying..." : "Verify & Sign in"}
                </Button>

                <div className="flex items-center justify-between text-sm pt-1">
                  <button type="button" onClick={() => { setStep("password"); setOtp(""); setError(""); setInfo(""); }}
                    className="flex items-center gap-1 text-white/50 hover:text-electric-light transition-colors">
        ...
        <p className="text-center text-xs text-white/40 mt-6">
          ...
          <Link href="/terms" className="text-electric-light hover:underline">Terms</Link> and{" "}
          <Link href="/privacy-policy" className="text-electric-light hover:underline">Privacy Policy</Link>
        </p>
      </div>
    </AuthShell>
  );
