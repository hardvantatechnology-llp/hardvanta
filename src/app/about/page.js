    ...
    <main className="relative min-h-screen overflow-hidden bg-gradient-to-b from-brand-silver to-brand-bg">
      ...
      <section className="relative bg-gradient-to-r from-brand-navy via-brand-blue to-brand-navy px-6 py-20 text-center text-white">
        ...
        <a
          ...
          className="mt-8 inline-block rounded-lg bg-gradient-to-r from-brand-blue to-brand-navy px-8 py-3 text-sm font-bold text-white shadow-brand-glow hover:brightness-110 transition-all"
        >
          Work With Us
        </a>
      </section>
          ...
          <div className="flex flex-wrap gap-6">
            {stats.map((s) => (
              <div key={s.label} className="text-center">
                <p className="text-lg font-extrabold text-brand-blue">{s.value}</p>
        ...
        <div className="grid gap-10 md:grid-cols-2 md:items-center">
          ...
          <div className="flex flex-col gap-4">
            {highlights.map((h) => (
              <div
                key={h.title}
                className="rounded-xl glass-brand-card p-5"
              >
                <h3 className="font-bold text-brand-text">{h.title}</h3>
                <p className="mt-1 text-sm text-brand-muted">{h.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative bg-white py-16">
        <div className="container-page">
          <p className="text-center text-xs font-bold uppercase tracking-widest text-brand-blue">
            What We Do
          </p>
      ...
      {/* ── Mission ── */}
      <section className="container-page relative py-16">
        <div className="rounded-2xl bg-gradient-to-r from-brand-navy via-brand-blue to-brand-navy px-8 py-12 text-center text-white">
      ...
      {/* ── Contact CTA ── */}
      <section className="relative border-t border-brand-border bg-brand-bg py-14 text-center">
        <p className="text-xs font-bold uppercase tracking-widest text-brand-blue">
          Get In Touch
        </p>
        <h2 className="mt-2 text-2xl font-extrabold text-brand-text">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-4">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="rounded-lg border border-brand-blue px-8 py-3 text-sm font-bold text-brand-blue hover:bg-brand-blue/5 transition-colors"
          >
            Visit Website →
          </a>
        </div>
