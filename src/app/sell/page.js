  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-gradient-to-b from-graphite to-obsidian">
      ...
      <section className="relative bg-gradient-to-r from-obsidian via-midnight to-obsidian px-6 py-16 text-center text-white">
        ...
        <a href="#apply" className="mt-6 inline-block rounded-lg bg-gradient-to-r from-electric to-liquid px-8 py-3 text-sm font-bold text-white shadow-glow-electric hover:brightness-110 transition-all">
          Start Selling
        </a>
      </section>
      ...
      <section className="container-page relative py-14">
        <h2 className="text-center text-2xl font-extrabold text-white">Why sell with us?</h2>
        <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {BENEFITS.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="rounded-2xl glass-card p-6">
              <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-electric/10 text-electric-light">
                <Icon size={22} />
              </div>
      ...
      <section className="relative bg-graphite py-14">
        <div className="container-page">
          <h2 className="text-center text-2xl font-extrabold text-white">How it works</h2>
          <div className="mt-8 grid gap-6 sm:grid-cols-3">
            {STEPS.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="rounded-2xl glass-card p-6 text-center">
                <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-r from-electric to-liquid text-white">
                  <Icon size={24} />
                </div>
                <h3 className="mt-4 font-bold text-white">{title}</h3>
                <p className="mt-1 text-sm text-white/60">{desc}</p>
      ...
      <section id="apply" className="container-page relative max-w-2xl py-14">
        <h2 className="text-2xl font-extrabold text-white">Seller Application</h2>
        <p className="mt-1 text-sm text-white/60">
          Fill in your details and our team will get in touch with the next steps.
        </p>

        {status === "done" ? (
          <div className="mt-8 flex flex-col items-center rounded-2xl glass-card p-10 text-center">
            ...
            <p className="mt-2 text-sm text-white/60">
              ...
              reach out to <span className="font-semibold text-white/90">{form.email}</span> soon.
            </p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="mt-8 space-y-4 rounded-2xl glass-strong p-8">
            ...
            <div>
              <label htmlFor="sell-message" className="mb-1 block text-sm font-medium text-white/80">Message (optional)</label>
              <textarea
                id="sell-message"
                rows={4}
                value={form.message}
            ...
            <button
              ...
              className="w-full rounded-lg bg-gradient-to-r from-electric to-liquid py-3 text-sm font-bold text-white shadow-glow-electric hover:brightness-110 transition-all disabled:opacity-60"
            >
